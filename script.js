...
document.addEventListener("DOMContentLoaded", () => {
  const shapes = document.querySelectorAll(".square, .circle");
  ...
  shapes.forEach(shape => {
    ...
    const randomX = Math.random() * (bgWidth - shapeWidth);
    const randomY = Math.random() * (bgHeight - shapeHeight);
...
// Toggle info sections when cards are clicked
document.querySelectorAll('.info-card').forEach(card => {
  card.addEventListener('click', () => {
    ...
    const offset = 20; // optional offset from top
